...
{
    if (scribble_autotype_get(text_elements[_i][2]) < 1.0)
    {
        _complete = false;
        break;
    }
...
if (_complete)
{
    ...
    if (chatterbox_is_waiting(chatterbox)) //If Chatterbox is waiting for the user to press space... then... check for... that..
    {
    ...
    else if (chatterbox_get_option_count(chatterbox)) //If Chatterbox is presenting the user with some options, check for that
    {
        ...
        if (_index != undefined)
        {
            ...
            chatterbox_select(chatterbox, _index);
            //Always update your text elements after advancing chatterbox!
            refresh_text_elements();
        }
    }
...
else
{
    ...
    if (keyboard_check_pressed(vk_space))
    {
        //...and the player has pressed space then tell Scribble to skip the fade in and just display all the text
        var _i = 0;
        repeat(array_length(text_elements))
        {
            scribble_autotype_skip(text_elements[_i][2]);
            ++_i;
        }
    }
}